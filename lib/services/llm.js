...
async function translateExplanation(provider, model, sysPrompt, explanation) {
  ...
  if (provider === 'openai') {
    ...
    const text = extractAssistantMessageText(json?.choices?.[0]?.message)
...
function coerceMessageFieldToText(value) {
  ...
  if (Array.isArray(value)) {
    ...
    for (const item of value) {
      ...
      else if (typeof item.value === 'string' && item.value) parts.push(item.value)
      else if (Array.isArray(item.content)) {
        const nested = coerceMessageFieldToText(item.content)
  ...
  if (value && typeof value === 'object') {
    ...
    if (Array.isArray(value.content)) return coerceMessageFieldToText(value.content)
    if (Array.isArray(value.parts)) return coerceMessageFieldToText(value.parts)
  }
  return ''
}
...
function extractAssistantMessageText(message) {
  ...
  const contentText = coerceMessageFieldToText(message.content)
  if (contentText) return contentText
  const reasoningText = coerceMessageFieldToText(message.reasoning)
...
async function callOpenRouter(input, indicators, horizon, model, sysPrompt, opts = {}) {
  ...
  const text = extractAssistantMessageText(choice && choice.message)
