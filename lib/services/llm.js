...
function buildPrompt(input, horizon, sysPrompt) {
  const header = [
    `Task: forecast the next ${horizon} trading days closing prices for a stock.`,
    `Rules: return exactly ${horizon} future trading days strictly after last_date; do not include any historical dates; skip weekends; dates must be strictly increasing; keep price path plausible and smooth relative to last_close and recent volatility.`,
    `Use recent_closes and summary (trend/volatility/support/resistance/regime) to guide the trajectory.`,
    `Output JSON only.`,
    `Write 3-8 short explanation points focusing on the drivers you used.`,
  ].join(' ')
...
async function callOpenRouter(input, horizon, model, sysPrompt, opts = {}) {
  ...
  const body = {
    ...
    temperature,
    max_tokens: maxTokens,
    response_format: predictJsonSchema(horizon)
  }
  let res
  try {
...
async function callOpenAI(input, horizon, model, sysPrompt, opts = {}) {
...
async function callDeepseek(input, horizon, model, sysPrompt, opts = {}) {
...
export function listModels() {
  return [
    { id: 'openrouter/free', name: '自动', type: 'chat', enabled: true, maxTokens: 2048 },
    { id: 'qwen/qwen3.6-plus-preview:free', name: 'Qwen3: Qwen3.6 Plus', type: 'chat', enabled: true, maxTokens: 2048 },
    { id: 'nvidia/nemotron-3-super-120b-a12b:free', name: 'NVIDIA: Nemotron 3 Super', type: 'chat', enabled: true, maxTokens: 4096 },
    { id: 'arcee-ai/trinity-large-preview:free', name: 'Arcee AI: Trinity Large Preview', type: 'chat', enabled: true, maxTokens: 2048 }
  ]
}
...
function pickMaxTokensFromModelList(modelId) {
  ...
  if (!id) return null
  const m = listModels().find(x => x && x.id === id) || null
...
export async function runLlmPredict(body) {
  ...
  if (body?.params?.model != null) {
    ...
    if (typeof pm === 'string') {
      ...
      if (pm.id) model = String(pm.id)
      else if (pm.model) model = String(pm.model)
      const n = Number(pm.maxTokens)
      if (Number.isFinite(n) && n > 0) maxTokensOverride = Math.floor(n)
    }
  }
  if (maxTokensOverride == null) maxTokensOverride = pickMaxTokensFromModelList(model)

  let r
  if (provider === 'openai') r = await callOpenAI(input, horizon, model, sysPrompt, { maxTokens: maxTokensOverride })
  else if (provider === 'deepseek') r = await callDeepseek(input, horizon, model, sysPrompt, { maxTokens: maxTokensOverride })
  else r = await callOpenRouter(input, horizon, model, sysPrompt, { maxTokens: maxTokensOverride })
  if (r && r.error) {
  ...
  return { data: { forecast, provider: r.model, explanation } }
}
